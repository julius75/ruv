...
class MoovMoneyController extends Controller
{
    ...
    public function initiateMoovPaymentBundle(Request $request)
    {
        ...
        try{
            ...
            $initiate_request = $this->send_request_to_moov_api_bundles($refNumber, $request->moov_cash_phone_number, $provider, $request->amount);
            ...
            $moov->amount = $request->amount;
            $moov->provider_id = $request->provider_id;
            $moov->remarks = "Buy RUV Bundles";
            $moov->message = "Buy $provider->name bundles worth $request->amount from RUV-BF";
            $moov->issued = false;
            if ($initiate_request['code'] == 200){
                $moov->status = true;
            }else{
            ...
            if ($device){
                $push_message = "You will receive your Orange bundles of CFA. $moov->amount.";
                SendPushNotification::dispatch($device->token, $push_message)->delay(10);
                return response()->json(['message'=>'Transaction Assigned and notification sent'], Response::HTTP_OK);
            }
            else{
